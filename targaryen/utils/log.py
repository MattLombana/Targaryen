...
logger = logging.getLogger('targaryen')
logger.setLevel(machines['logging']['level'])

formatter = logging.Formatter(
    '%(asctime)s : %(threadName)-11s : %(levelname)7s : %(message)s (%(source)s)')

# Main Log File
filehandler = logging.FileHandler(log_path.join('targaryen.log').strpath, 'a')
filehandler.setLevel(machines['logging']['level'])
filehandler.setFormatter(formatter)
logger.addFilter(_RelpathFilter())
logger.addHandler(filehandler)
logger.propagate = False

# Log warnings to targaryen logger
